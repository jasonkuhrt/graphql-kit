...
// Type changes
TypeAdded, TypeRemoved, TypeKindChanged, TypeDescriptionChanged, TypeDescriptionAdded, TypeDescriptionRemoved, 
// Field changes
FieldAdded, FieldRemoved, FieldTypeChanged, FieldDeprecationAdded, FieldDeprecationRemoved, FieldDeprecationReasonChanged, FieldDeprecationReasonAdded, FieldDeprecationReasonRemoved, FieldDescriptionChanged, FieldDescriptionAdded, FieldDescriptionRemoved, 
// Field argument changes
FieldArgumentAdded, FieldArgumentRemoved, FieldArgumentTypeChanged, FieldArgumentDescriptionChanged, FieldArgumentDefaultChanged, 
// Enum changes
EnumValueAdded, EnumValueRemoved, EnumValueDescriptionChanged, EnumValueDeprecationAdded, EnumValueDeprecationRemoved, EnumValueDeprecationReasonChanged, EnumValueDeprecationReasonAdded, EnumValueDeprecationReasonRemoved, 
// Input field changes
InputFieldAdded, InputFieldRemoved, InputFieldTypeChanged, InputFieldDescriptionAdded, InputFieldDescriptionRemoved, InputFieldDescriptionChanged, InputFieldDefaultValueChanged, 
// Union changes
UnionMemberAdded, UnionMemberRemoved, 
// Interface changes
ObjectTypeInterfaceAdded, ObjectTypeInterfaceRemoved, 
// Directive changes
DirectiveAdded, DirectiveRemoved, DirectiveDescriptionChanged, DirectiveLocationAdded, DirectiveLocationRemoved, DirectiveArgumentAdded, DirectiveArgumentRemoved, DirectiveArgumentDescriptionChanged, DirectiveArgumentDefaultValueChanged, DirectiveArgumentTypeChanged, 
// Schema changes
SchemaQueryTypeChanged, SchemaMutationTypeChanged, SchemaSubscriptionTypeChanged).annotations({
    identifier: 'Change',
    title: 'Schema Change',
...
// Criticality guards
export const isBreaking = (change) => Criticality.isBreaking(change.criticality);
export const isDangerous = (change) => Criticality.isDangerous(change.criticality);
export const isSafe = (change) => Criticality.isSafe(change.criticality);
// ============================================================================
// Codec
// ============================================================================
 ...
 */
export const match = (change) => ({
    with: (tag, handler) => {
        if (change._tag === tag) {
            return handler(change);
        }
        return {
            with: (nextTag, nextHandler) => match(change).with(nextTag, nextHandler),
            otherwise: (defaultHandler) => defaultHandler(change),
        };
    },
    otherwise: (defaultHandler) => defaultHandler(change),
});
...
export const getCategory = (change) => {
    ...
    if (tag.startsWith('FIELD_'))
        return 'field';
    if (tag.startsWith('ENUM_'))
        return 'enum';
    if (tag.startsWith('INPUT_'))
        return 'input';
    if (tag.startsWith('UNION_'))
        return 'union';
    if (tag.startsWith('OBJECT_TYPE_INTERFACE_'))
        return 'interface';
    if (tag.startsWith('DIRECTIVE_'))
        return 'directive';
    if (tag.startsWith('SCHEMA_'))
        return 'schema';
...
export const getType = (change) => {
    ...
    if (tag === 'FIELD_DEPRECATION_REASON_CHANGED' || tag === 'FIELD_DEPRECATION_REASON_ADDED'
    ...
    if (tag === 'ENUM_VALUE_DEPRECATION_REASON_CHANGED' || tag === 'ENUM_VALUE_DEPRECATION_REASON_ADDED'
    ...
    if (tag === 'INPUT_FIELD_DESCRIPTION_ADDED' || tag === 'INPUT_FIELD_DESCRIPTION_REMOVED'
    ...
    // Schema changes
    if (tag === 'SCHEMA_QUERY_TYPE_CHANGED' || tag === 'SCHEMA_MUTATION_TYPE_CHANGED'
