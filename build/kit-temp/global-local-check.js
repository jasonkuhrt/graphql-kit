 ...
 */
const findPackageInAncestors = (packageName) => Effect.gen(function* () {
    let dir = process.cwd();
    while (true) {
 ...
 */
function createGlobalLocalConflictError(packageName, projectDir, template) {
 ...
 * ```
 */
export const checkGlobalVsLocal = (options) => Effect.gen(function* () {
 ...
 */
export async function checkGlobalVsLocalAsync(options) {
    const { NodeFileSystem } = await import('@effect/platform-node');
    return Effect.runPromise(Effect.provide(checkGlobalVsLocal(options), NodeFileSystem.layer));
}
//# sourceMappingURL=global-local-check.js.map