...
// Schema Definition
// ============================================================================
export const Path = S.transformOrFail(S.String, Nodes.Root.Schema, {
    strict: true,
    decode: (input) => ParseResult.try({
        try: () => parse(input),
        catch: (error) => new ParseResult.Type(S.String.ast, input, error instanceof Error ? error.message : String(error)),
    }),
...
export function decode(input) {
    return S.decodeUnknown(Path)(input);
}
/**
 ...
 * // Type is exactly: ParsePath<'User.posts'>
 */
export function decodeSync(input) {
    return S.decodeUnknownSync(Path)(input);
}
