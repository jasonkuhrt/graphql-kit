...
export const GraphqlSchema = define({
    introspection: {
        getKind({ node }) {
            if (Grafaid.Schema.TypesLike.isNamed(node)) {
                return Grafaid.Schema.typeClassNameToKindMap[Grafaid.Schema.typeClassToName(node)];
            }
        },
        getName({ node }) {
            if (Grafaid.Schema.TypesLike.isNamed(node)) {
    ...
    .GraphQLPathSegmentType($ => $.step(({ pathNode, context }) => {
    ...
    const arg = targetNode.args.find((a) => a.name === pathNode.name);
    if (!arg) {
        return Either.left(StepFailures.NodeNotFound({}));
    }
    return Either.right(arg);
}))
    .GraphQLPathSegmentResolvedType($ => $.step(({ targetNode }) => {
