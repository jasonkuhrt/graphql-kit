 ...
 */
const extractTypeNameFromChange = (change) => {
    return Match.value(change).pipe(Match.tag('TYPE_ADDED', (c) => Option.some(c.name)), Match.tag('TYPE_REMOVED', (c) => Option.some(c.name)), Match.tag('FIELD_ADDED', (c) => Option.some(c.typeName)), Match.tag('FIELD_REMOVED', (c) => Option.some(c.typeName)), Match.tag('INPUT_FIELD_ADDED', (c) => Option.some(c.inputName)), Match.tag('INPUT_FIELD_REMOVED', (c) => Option.some(c.inputName)), Match.tag('ENUM_VALUE_ADDED', (c) => Option.some(c.enumName)), Match.tag('ENUM_VALUE_REMOVED', (c) => Option.some(c.enumName)), Match.tag('UNION_MEMBER_ADDED', (c) => Option.some(c.unionName)), Match.tag('UNION_MEMBER_REMOVED', (c) => Option.some(c.unionName)), Match.tag('OBJECT_TYPE_INTERFACE_ADDED', (c) => Option.some(c.objectName)), Match.tag('OBJECT_TYPE_INTERFACE_REMOVED', (c) => Option.some(c.objectName)), Match.orElse(() => Option.none()));
};
/**
 * Build lifecycle data from a single schema
 */
export const createFromSchema = (schema) => {
    const lifecycles = {};
 ...
 */
export const getTypeSince = (lifecycles, typeName, currentSchema) => {
    ...
    for (const entry of entries) {
        if (entry.change._tag === 'TYPE_ADDED' && entry.change.name === typeName) {
            ...
            if (currentSchema && entry.schema._tag === 'SchemaVersioned') {
                if (currentSchema._tag !== 'SchemaVersioned')
 ...
 */
export const getTypeAddedDate = (lifecycles, typeName, currentVersion) => {
    const entries = lifecycles[typeName];
    if (!entries)
        return undefined;
 ...
 */
export const getTypeRemovedDate = (lifecycles, typeName, currentSchema) => {
    const entries = lifecycles[typeName];
    if (!entries)
        return undefined;
 ...
 */
export const isTypeCurrentlyAvailable = (lifecycles, typeName) => {
    const entries = lifecycles[typeName];
    if (!entries || entries.length === 0)
        return false;
 ...
 */
export const getFieldSince = (lifecycles, typeName, fieldName, currentSchema) => {
    const entries = lifecycles[typeName];
 ...
 */
export const getFieldAddedDate = (lifecycles, typeName, fieldName, currentSchema) => {
    const entries = lifecycles[typeName];
    if (!entries)
        return undefined;
    // Get current version from schema if provided
 ...
 * For changelog: Shows removal date if it happened in any version
 */
export const getFieldRemovedDate = (lifecycles, typeName, fieldName, currentSchema) => {
    const entries = lifecycles[typeName];
    if (!entries)
        return undefined;
    // If no current schema specified (e.g., in changelog), show any removal
 ...
 */
export const isFieldCurrentlyAvailable = (lifecycles, typeName, fieldName) => {
    const entries = lifecycles[typeName];
    if (!entries || entries.length === 0)
        return false;
 ...
 */
export const getFieldLifecycle = (lifecycles, typeName, fieldName) => {
    const entries = lifecycles[typeName];
    if (!entries)
        return Option.none();
 ...
 */
export const getTypeLifecycle = (lifecycles, typeName) => {
    const entries = lifecycles[typeName];
    if (!entries)
        return Option.none();
